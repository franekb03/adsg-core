...
class BeamEvaluator(StochasticDSGEvaluator):
    ...
    def __init__(self, uq_method=None, stress_ref=None, **kwargs):
        ...
        self.par_load = InputParameterNode('load', NormalDistribution(100., 20.))
        self.par_rho = InputParameterNode('rho_factor', 1.5)  # deterministic
        self.par_e = {'steel': InputParameterNode('E_steel', NormalDistribution(210., 10.)),
                      'alu': InputParameterNode('E_alu', NormalDistribution(70., 10.))}
...
def test_input_parameter_node(n):
    stochastic = InputParameterNode('E', NormalDistribution(10., 2.))
    ...
    assert InputParameterNode('h', UniformDistribution(0., 1.)).is_stochastic
    ...
    assert InputParameterNode('E', NormalDistribution(10., 2.)) != stochastic
    assert len({stochastic, InputParameterNode('E', NormalDistribution(10., 2.))}) == 2


def test_set_get_input_parameter_value(n):
    par_a = InputParameterNode('A', NormalDistribution(0., 1.))
...
def test_parameter_node_conditional_existence(n):
    common = InputParameterNode('common', NormalDistribution(0., 1.))
    only_a = InputParameterNode('only_a', NormalDistribution(1., 1.))
    only_b = InputParameterNode('only_b', NormalDistribution(2., 1.))
...
def test_param_space_holds_only_stochastic_parameters(n):
    stochastic = InputParameterNode('u', NormalDistribution(0., 1.))
    deterministic = InputParameterNode('rho', 1.225)
...
def test_param_space_builds_the_openturns_distributions(n):
    ...
    normal = InputParameterNode('u', NormalDistribution(10., 2.))
    uniform = InputParameterNode('h', UniformDistribution(4., 10.))
...
def test_param_realization_is_keyed_by_node(n):
    ...
    stochastic = InputParameterNode('u', NormalDistribution(10., 2.))
    deterministic = InputParameterNode('rho', 1.225)
    processor = GraphProcessor(_dsg_with_parameters(n, [stochastic, deterministic]))
